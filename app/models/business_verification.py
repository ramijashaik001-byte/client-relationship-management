# app/models/business_verification.py
import datetime
...
class BusinessVerification(Base):
    ...
    registry_status = Column(String(100), nullable=True)
    verified_data_raw = Column(Text, nullable=True)
    ...
    def __repr__(self) -> str:
        return f'<BusinessVerification(id={self.id})>'
    ...
    def validate_entity_state(self) -> bool:
        ...
        if not self.status:
            return False
        if not self.incorporation_country:
            return False
        return True
