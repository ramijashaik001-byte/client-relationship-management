...
class FormTemplateBase(BaseModel):
    """Base pydantic schema representing shared attributes of FormTemplate."""
...
class FormTemplateCreate(FormTemplateBase):
    """Schema used during client creation requests for FormTemplate."""
    pass

class FormTemplateUpdate(BaseModel):
    """Schema representing properties that can be updated in FormTemplate."""
...
class FormTemplate(FormTemplateInDBBase):
    """API response model wrapper for FormTemplate."""
    pass

class FormTemplateDetailedList(BaseModel):
    items: List[FormTemplate]
    total_count: int
    page: int
    size: int
    pages_count: int

class FormTemplateAuditHistory(BaseModel):
    entity_id: int
