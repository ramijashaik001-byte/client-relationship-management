# app/schemas/document_signature.py
import datetime
from pydantic import BaseModel, Field
from typing import Optional, List, Dict, Any

class DocumentSignatureBase(BaseModel):
    """Base pydantic schema representing shared attributes of DocumentSignature."""
    document_id: int = Field(...)
    signer_name: str = Field(..., max_length=255)
    signer_email: str = Field(..., max_length=255)
    signature_status: str = Field('PENDING', max_length=50)
    ip_address: Optional[str] = Field(None, max_length=50)
    signature_timestamp: Optional[datetime.datetime] = Field(None)
    token: str = Field(..., max_length=100)

class DocumentSignatureCreate(DocumentSignatureBase):
    """Schema used during client creation requests for DocumentSignature."""
    pass

class DocumentSignatureUpdate(BaseModel):
    """Schema representing properties that can be updated in DocumentSignature."""
...
class DocumentSignatureInDBBase(DocumentSignatureBase):
    id: int
...
class DocumentSignature(DocumentSignatureInDBBase):
    """API response model wrapper for DocumentSignature."""
    pass

class DocumentSignatureDetailedList(BaseModel):
    items: List[DocumentSignature]
    total_count: int
    page: int
    size: int
    pages_count: int

class DocumentSignatureAuditHistory(BaseModel):
    entity_id: int
